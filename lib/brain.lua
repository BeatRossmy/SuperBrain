BRAIN = {
  lp = LP_X.new(1,midi.connect(1),false,127),
  ...
  init = function (self)
    ...
    self.lp:all(0)
    self.lp:refresh()
    self.lp.key = function (x,y,z) self:key(x,y,z) end
  ...
  redraw = function (self)
    ...
    -- ========================== --
    -- TRANSPORT
      self.lp:led(10,1,self.transport_state=="play" and 15 or 2)
      self.lp:led(10,2,2)
    
    -- TRACK SELECTION
    for i=1,#self.tracks do
      self.lp:led(10,8-#self.tracks+i,i==self.focus and 15 or 2)
    end
    
    if self.ui_mode=="apps" then
      -- FOCUSSED TRACK
      if sel_tr.visible and sel_tr.engine then sel_tr.engine:redraw(self.lp) end
      -- KEYBOARD
      if self.keys.visible then self.keys:redraw(self.lp) end
    elseif self.ui_mode=="settings" then
      sel_tr:show_settings(self.lp)
    elseif self.ui_mode=="presets" then
      ...
      for i=1,64 do
        ...
        if util.file_exists (file_path) then
          ...
          self.lp:led(pos.x,pos.y,3)
        end
      end
      local pos = index_to_pos[self.preset]
      self.lp:led(pos.x,pos.y,5,"fade")
    end
